...
"""
Web 控制面板。
优先加载拆分后的 routes_*.py；若缺模块则回退到历史完整版 app 下载。
"""
import os
import sys
import logging
...
def _has_split_modules() -> bool:
    ...
    return all(os.path.isfile(os.path.join(_DIR, n)) for n in required)
...
if _has_split_modules():
    ...
    logger.info("[web] split modules incomplete, using monolith fallback")
    _run_monolith()
